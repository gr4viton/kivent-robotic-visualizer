    ...
    def load_svg(self, fname, massless=False):
        ...
        for info in data["model_info"]:
            ...
            if info.description is not None:
                ...
                collision_type = int(
                    info_dict.get("collision_type", collision_type)
                )
            # if massless:
            #    mass = float('inf')
            #    mass = float(0)
            ...
            # Logger.debug
            prind(
                "adding object with title/element_id=%s/%s and desc=%s",
                info.title,
                info.element_id,
                info.description,
            )
            ...
            self.root.init_entity(
                create_dict, component_order, object_info=object_info
            )
